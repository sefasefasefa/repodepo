    ...
    <>
      ...
      <aside className={cn(
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain">
          ...
          {nav("videos").enabled && (
            <Row icon={PlayCircle} label={nav("videos").label || "Videolar"} href="/videos?sort=trending" onClick={onClose} active={location === "/videos"} />
          )}
          {nav("shorts").enabled && (
            <Row icon={FastForward} label={nav("shorts").label || "Shorts"} href="/shorts" onClick={onClose} active={location === "/shorts"} />
          )}
          {nav("recommended").enabled && (
            <Row icon={ThumbsUp} label={nav("recommended").label || "Önerilenler"} href="/videos?sort=most_liked" onClick={onClose} />
          )}
          {nav("trending").enabled && (
            <Row icon={Flame} label={nav("trending").label || "Trend"} href="/videos?sort=trending" onClick={onClose} flag="🇹🇷" />
          )}

          {nav("models").enabled && (
            <>
              <Row icon={Star} label={nav("models").label || "Modeller"} expandable expanded={modelsExpanded} onToggle={() => setModelsExpanded(p => !p)} />
              {modelsExpanded && (
          ...
          {nav("channels").enabled && (
            <Row icon={Tv} label={nav("channels").label || "Kanallar"} href="/creators" onClick={onClose} />
          )}

          {nav("top-cats").enabled && (
            <>
              <Row icon={TrendingUp} label={nav("top-cats").label || "Top Kategoriler"} expandable expanded={topCatsExpanded} onToggle={() => setTopCatsExpanded(p => !p)} />
              {topCatsExpanded && (
                <div className="bg-[#161616]">
          ...
          {nav("all-categories").enabled && (
            <Link href="/categories" onClick={onClose}>
              <div className="mx-4 my-3 py-2.5 bg-[#2a2a2a] hover:bg-[#333] transition-colors rounded text-center text-sm text-[#ccc] font-medium border border-[#383838] cursor-pointer">
                {nav("all-categories").label || "Tüm Kategoriler"}
              </div>
            </Link>
          )}

          {nav("playlists").enabled && (
            <Row icon={ListVideo} label={nav("playlists").label || "Oynatma Listeleri"} href="/playlists" onClick={onClose} active={location === "/playlists"} />
          )}
          {nav("stories").enabled && (
            <Row icon={Image} label={nav("stories").label || "Hikayeler"} href="/stories" onClick={onClose} active={location === "/stories"} />
          )}
          ...
          {nav("photos").enabled && (
            <Row icon={Image} label={nav("photos").label || "Fotoğraflar"} href="/videos" onClick={onClose} />
          )}

          {nav("community").enabled && (
            <>
              <Row icon={Users} label={nav("community").label || "Topluluk"} expandable expanded={communityExpanded} onToggle={() => setCommunityExpanded(p => !p)} />
          ...
          {user && sec("section-account").enabled && (
            <>
              <SectionDivider label={sec("section-account").label || "Hesabım"} />
              {nav("history").enabled && <Row icon={History} label={nav("history").label || "İzleme Geçmişi"} href="/history" onClick={onClose} active={location === "/history"} />}
              {nav("bookmarks").enabled && <Row icon={Bookmark} label={nav("bookmarks").label || "Kaydedilenler"} href="/bookmarks" onClick={onClose} active={location === "/bookmarks"} />}
              {nav("notifications").enabled && <Row icon={Bell} label={nav("notifications").label || "Bildirimler"} href="/notifications" onClick={onClose} active={location === "/notifications"} />}
          ...
          {isCreator && sec("section-creator").enabled && (
            <>
              <SectionDivider label={sec("section-creator").label || "İçerik Paneli"} />
              {nav("upload").enabled && <Row icon={PlusCircle} label={nav("upload").label || "Video Yükle"} href="/upload" onClick={onClose} />}
              {nav("creator-dash").enabled && <Row icon={LayoutDashboard} label={nav("creator-dash").label || "Dashboard"} href="/creator/dashboard" onClick={onClose} />}
            </>
          )}
          ...
          {isAdmin && sec("section-admin").enabled && (
            <>
              <SectionDivider label={sec("section-admin").label || "Yönetim"} />
              {nav("admin-panel").enabled && <Row icon={ShieldAlert} label={nav("admin-panel").label || "Admin Paneli"} href="/admin" onClick={onClose} />}
            </>
          )}
          ...
          {categories.filter(c => (c.videoCount ?? 0) > 0).length > 0 && sec("section-categories").enabled && (
            <>
              <SectionDivider label={sec("section-categories").label || "Kategoriler"} />
              {categories.filter(c => (c.videoCount ?? 0) > 0).map((cat: any) => (
          ...
          {nav("personalized").enabled && (
            <div className="flex items-center justify-between px-4 py-4 border-t border-[#2c2c2c] mt-1">
              <span className="text-[14px] text-[#ccc]">{nav("personalized").label || "Kişiselleştirilmiş"}</span>
