...
import { Crown, CheckCircle2, Calendar, CreditCard, Zap, ArrowRight, Sparkles, Clock } from "lucide-react";
...
const PLAN_PERKS: Record<string, string[]> = {
  ...
  premium: [
    ...
    "Aylık 50 bonus token",
  ],
  vip: [
    "Reklamsız izleme",
    "8K + Canlı yayın kalitesi",
    "Sınırsız video + Arşiv",
    "Sınırsız indirme",
    "7/24 Öncelikli destek",
    "Tüm özel içerikler",
    "Aylık 200 bonus token",
    "Creator mesajlarına öncelik",
    "VIP rozeti",
  ],
};
...
export default function Subscriptions() {
  ...
  const perks = PLAN_PERKS[planName] ?? PLAN_PERKS.basic;
  const renewDate = sub ? new Date(sub.currentPeriodEnd).toLocaleDateString("tr-TR", {
          ...
          <>
              ...
              <div className="p-6 space-y-5">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    <div className="flex items-center gap-2.5 mb-1">
                      <Crown className="h-5 w-5 text-yellow-400" />
                      <h2 className="text-xl font-bold text-white">{sub.plan.name} Plan</h2>
                      <span className={cn(
                        "text-[10px] font-bold px-2 py-0.5 rounded-full border",
                        STATUS_STYLE[sub.status] ?? STATUS_STYLE.active
                      )}>
                  ...
                  {(sub as any).amount && (
                    <div className="text-right">
                      <p className="text-2xl font-black text-white">${(sub as any).amount}<span className="text-sm text-[#666] font-normal">/ay</span></p>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-3.5">
                    ...
                    <p className="text-sm font-bold text-green-400">{STATUS_LABEL[sub.status] ?? "—"}</p>
                  </div>
                </div>
            ...
            <div className="flex items-start gap-3 bg-primary/5 border border-primary/20 rounded-xl p-4">
              ...
              <div>
                ...
                <p className="text-xs text-[#888] mt-0.5">Premium özel içerikleri keşfetmek için <Link href="/videos?isPremium=true"><span className="text-primary underline cursor-pointer">buraya tıkla</span></Link>. Tokenlerin ile yaratıcılara bahşiş verebilirsin.</p>
              </div>
            </div>
          </>
        ) : (
